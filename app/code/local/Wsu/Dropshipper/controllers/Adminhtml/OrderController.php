            ...
            try {
                foreach ($data['dropshipper'] as $key => $_dropshipper) {
                    if ($_dropshipper['id'] && $_dropshipper['item_id']) {
                        $dropshipper = Mage::getModel('wsu_dropshipper/order')->getCollection()->getItemByColumnValue('item_id', $_dropshipper['item_id']);
                        if (!$dropshipper) {
                            $newData                = array();
                            ...
                            $dropshipper->setDshipperId($_dropshipper['id'])->save();
                        }
                    }
                }
                ...
                return $this->_redirect('adminhtml/sales_order/view', array(
                    ...
                    '_current' => true,
                    'tab' => 'sales_order_view_tabs_dropshipper'
                ));
            }
                ...
                if ($dropshipperMailOption) {
                    ...
                    if (!empty($_allDropShippers)) {
                        foreach ($_allDropShippers as $_dropshipper) {
                            ...
                            $_itemCollection = Mage::getResourceModel('sales/order_item_collection')->addFieldToFilter('item_id', array(
                                'in' => $items
                            ...
                            $mailTemplate->setDesignConfig(array(
                                'area' => 'frontend',
                                'store' => $storeId
                            ))->sendTransactional($templateId, array(
                                'email' => Mage::getStoreConfig(self::XML_PATH_EMAIL_DROPSHIPPER_SENDER),
                                'name' => Mage::getStoreConfig(self::XML_PATH_EMAIL_DROPSHIPPER_SENDER_NAME)
                            ), $_dropshipper['email'], $_dropshipper['name'], array(
                                'order' => $order,
